...
admin = {'admin': 'admin', 'carkson': 'carkson'}
...
def reminder():
    ...
    for x in scheduled:
        ...
        if time_clock > time_hrs:
            ...
            tk.Label(master=scheduled_reminders, text="Flight Number " + x + " " "destination: England" + " Not Updated").grid(row=0, column=0)
...
def cancel(): 
    ...
    admin7 = tk.Button(master=cancelled_scheduled, width=25, text="Confirm", command=back_cancelled).grid(row=2, column=1)
...
def manager_main():  
    ...
    image = tk.PhotoImage(file="icon1.png")
    ...
    tk.Button(master=manager_main_scheduled, text="View The Details Of Flights", command=viewing_flights).grid(row=1,column=1)
...
def viewing_flights(): 
    ...
    tk.Label(master=display, text="Flight Number--------Time--------Destination-------Status").grid(row=1, column=0)
    for i in scheduled:
        can += 1
        ret += 1
...
def main_standard(): 
    ...
    tk.Button(master=main_standard_scheduled, text="View The Details Of Flights", command=viewing_flights).grid(row=2, column=0)
    ...
    img = tk.PhotoImage(file="icon1.png")
...
def login():  
    def user_verification():
        ...
        if not (a in admin or a in manager or a in standard):
            ...
            admin3 = tk.Button(master=login_sched, text="Confirm Password", width=25,command=password_verification).grid(row=2, column=2)
                               

    login_sched = tk.Tk()
    login_sched.title("Carkson flight management Login page")
    ...
    username.grid(row=1, column=1)
    asking = tk.Button(master=login_sched, text="Confirm Username", width=25, command=user_verification).grid(row=1,column=2)
